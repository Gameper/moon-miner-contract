...
//        locked[id] = true;
    ...
    function mine(uint256 _id, uint256 _nonce, bytes32 _challenge_digest) public returns (bool) {
        ...
        ratio == 0 ? holderTreasureAmount = 0 : holderTreasureAmount= minedTreasureAmount.div(5).div(uint256(1000).div(ratio));
//        uint256 minerTreasureAmount = minedTreasureAmount.sub(holderTreasureAmount);
    ...
    function getLastMiner(uint256 _id) public view returns (address) {
        MiningInfo memory miningInfo = miningInfos[_id];
        return miningInfo.lastRewardTo;
    }
